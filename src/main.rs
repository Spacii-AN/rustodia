            ...
            .output()
        {
            if let Ok(name) = String::from_utf8(output.stdout) {
                ...
                return name
                    .as_bytes()
                    .windows(8)
                    .any(|w| w.eq_ignore_ascii_case(b"warframe"));
            }
        }
...
fn button_from_index(idx: usize) -> enigo::Button {
    ...
    if idx == 8 {
        ...
        BUTTON_LOOKUP
            .get(idx)
            .and_then(|&b| b)
            .unwrap_or(enigo::Button::Left)
    }
}

#[inline(always)]
fn device_query_button_pressed(buttons: &[bool], idx: usize) -> bool {
    buttons.get(idx).copied().unwrap_or(false)
}

#[cfg(target_os = "linux")]
struct LinuxSideButtonState {
    side_pressed: AtomicBool,
    extra_pressed: AtomicBool,
}

#[cfg(target_os = "linux")]
impl LinuxSideButtonState {
    fn new() -> Self {
        Self {
            side_pressed: AtomicBool::new(false),
            extra_pressed: AtomicBool::new(false),
        }
    }

    fn update(&self, code: u16, pressed: bool) {
        match code {
            275 => self.side_pressed.store(pressed, Ordering::Relaxed),
            276 => self.extra_pressed.store(pressed, Ordering::Relaxed),
            _ => {}
        }
    }
    ...
    fn is_pressed(&self, idx: usize) -> bool {
        match idx {
            8 => self.side_pressed.load(Ordering::Relaxed),
            9 => self.extra_pressed.load(Ordering::Relaxed),
            _ => false,
        }
    }
}
...
#[cfg(target_os = "linux")]
fn start_linux_side_button_monitor() -> Option<Arc<LinuxSideButtonState>> {
    use std::fs;
        ...
        for entry in entries.flatten() {
            ...
            if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                ...
                match Device::open(&path) {
                    Ok(device) => {
                        let device_name = device.name().unwrap_or_default().to_string();
                        let name_lower = device_name.to_lowercase();
                        if name_lower.contains("mouse") || name_lower.contains("pointer") {
                            eprintln!(
                                "Linux side button monitor: using {} ({})",
                                name, device_name
                            );
                    ...
                    Err(err) => {
                        if err.kind() != ErrorKind::PermissionDenied {
                            eprintln!("Warning: could not open {}: {}", name, err);
                        }
                    }
                }
            }
    ...
    let device = match found_device {
        ...
        None => {
            eprintln!(
                "⚠️  Linux: unable to access a /dev/input event device for side button detection."
            );
            eprintln!("   Side buttons may require joining the 'input' group (sudo usermod -aG input $USER).");
            return None;
        }
    };

    let state = Arc::new(LinuxSideButtonState::new());
                ...
                Ok(events) => {
                    for event in events {
                        if event.event_type().0 == 1 {
                            ...
                            monitor_state.update(event.code(), pressed);
                        }
                    }
                }
                Err(err) => {
                    if err.kind() != ErrorKind::WouldBlock {
                        eprintln!("Linux side button monitor error: {}", err);
...
fn execute_contagion_sequence(
    ...
    let (
        ...
        melee_hold_time,
        emote_prep_delay,
        rapid_fire_click_delay,
        sequence_end_delay,
        _,
        _,
    ) = get_durations_from_config(config);
...
fn run_macro(config: Arc<Mutex<SharedConfig>>, state: Arc<State>) {
    ...
    println!(
        "  - Press '{}' to perform {} rapid clicks",
        config_snapshot.rapid_click_key, config_snapshot.rapid_click_count
    );
    println!("  - Press F11 to toggle all macros on/off");
    println!("\nPress Ctrl+C to exit\n");

    #[cfg(target_os = "linux")]
    let linux_side_buttons = {
        let monitor = start_linux_side_button_monitor();
        if monitor.is_none() {
            eprintln!("Linux side button detection will rely on device_query only (button8/button9 may not trigger).");
        }
        monitor
    };
    ...
    #[cfg(target_os = "linux")]
    let linux_side_buttons_clone = linux_side_buttons.clone();
    thread::spawn(move || {
        #[cfg(target_os = "linux")]
        let linux_side_buttons = linux_side_buttons_clone;
        ...
        loop {
            ...
            if warframe_active {
                ...
                if rapid_click_pressed
                    && !last_rapid_click_state
                ...
                let mut macro_pressed =
                    device_query_button_pressed(&mouse.button_pressed, macro_button_idx);

                #[cfg(target_os = "linux")]
                {
                    if let Some(ref side_buttons) = linux_side_buttons {
                        macro_pressed |= side_buttons.is_pressed(macro_button_idx);
                    }
                }

                if let Some(alt_idx) = keybinds.macro_alt {
                    let mut alt_pressed =
                        device_query_button_pressed(&mouse.button_pressed, alt_idx);

                    #[cfg(target_os = "linux")]
                    {
                        if let Some(ref side_buttons) = linux_side_buttons {
                            alt_pressed |= side_buttons.is_pressed(alt_idx);
                        }
                    }

                    macro_pressed |= alt_pressed;
                }
...
fn main() {
    ...
    if use_gui {
        ...
        println!(
            "Note: Keybind configuration works independently - Warframe does not need to be open"
        );
